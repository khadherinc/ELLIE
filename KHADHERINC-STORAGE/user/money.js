  const fs = require('fs')
  let _monayOrg = JSON.parse(fs.readFileSync('./KHADHERINC-STORAGE/user/money.json'))
  let monayAwal = global.limitawal.monayawal
  const addInventoriMonay = (sender) => {
        const obj = {id: sender, monay: monayAwal}
         _monayOrg.push(obj)
        fs.writeFileSync('./KHADHERINC-STORAGE/user/money.json', JSON.stringify(_monayOrg))
   }
