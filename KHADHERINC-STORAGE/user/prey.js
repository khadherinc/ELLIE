  const fs = require('fs')
  let _hasilBuruan = JSON.parse(fs.readFileSync('./KHADHERINC-STORAGE/user/bounty.json'))
  
  const addInventoriBuruan = (sender) => {
        const obj = {id: sender, ikan: 0, ayam: 0, kelinci: 0, domba: 0, sapi: 0, gajah: 0}
         _hasilBuruan.push(obj)
        fs.writeFileSync('./KHADHERINC-STORAGE/user/bounty.json', JSON.stringify(_hasilBuruan))
   }
