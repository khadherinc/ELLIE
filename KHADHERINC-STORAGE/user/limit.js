  const fs = require('fs')
  let _limitOrg = JSON.parse(fs.readFileSync('./KHADHERINC-STORAGE/user/limit.json'))
  let limitAwal = global.limitawal.free
  const addInventoriLimit = (sender) => {
        const obj = {id: sender, limit: limitAwal}
         _limitOrg.push(obj)
        fs.writeFileSync('./KHADHERINC-STORAGE/user/limit.json', JSON.stringify(_limitOrg))
   }
