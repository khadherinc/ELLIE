...
async function startA17() {
  ...
  console.log(color('\nHello, I am Khadher, the Developer of this bot.\n\nThanks for using: ELLIE .', 'aqua'))
  console.log(color('\nYou can follow me on GitHub: khadherinc', 'aqua'))

  const { state, saveCreds } = await useMultiFileAuthState("./KHADHERINC-SESSION");
  const A17 = A17Connect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["ELLIE WAAUTO", "Safari", "3.O"],
    auth: state,
  });
 ...
 A17.ev.on('groups.update', async pea => {
        ...
        ppgc = 'https://wallpapercave.com/wp/wp12002210.jpg'
        }
 ...
 */
     ...
     A17.ev.on('groups.update', async pea => {
      ...
      let wm_fatih = { url: ppgc }
      if (pea[0].announce == true) {
        //A17.send5ButImg(pea[0].id, `Group has been *Closed!* Only *Admins* can send Messages!`, `ELLIE `, wm_fatih, [])
  
        A17.sendMessage(m.chat, { image: wm_fatih, caption: 'Group has been *Closed!* Only *Admins* can send Messages!' })
      } else if (pea[0].announce == false) {
        // A17.send5ButImg(pea[0].id, `Grop has been *Opened!* Now *Everyone* can send Messages!`, `ELLIE `, wm_fatih, [])
        A17.sendMessage(m.chat, { image: wm_fatih, caption: 'Group has been *Opened!* Now *Everyone* can send Messages!' })
      } else if (pea[0].restrict == true) {
        //A17.send5ButImg(pea[0].id, `Group Info modification has been *Restricted*, Now only *Admins* can edit Group Info !`, `ELLIE `, wm_fatih, [])
        ...
        //A17.send5ButImg(pea[0].id, `Group Info modification has been *Un-Restricted*, Now only *Everyone* can edit Group Info !`, `ELLIE `, wm_fatih, [])
  ...
  I am *ELLIE*, Welcome to ${metadata.subject}.
